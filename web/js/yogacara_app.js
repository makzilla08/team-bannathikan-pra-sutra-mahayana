  ...
  return `
    <div class="sutra-card" onclick="location.href='reader.html?sutra=${sutraId}'" style="cursor:pointer">
      <div class="sutra-card-header">
        <div class="sutra-number">📖 โยคาจาร</div>
...
function loadTexts(containerId) {
  ...
  if (!el) return;
  if (YOGACARA_DATA.length === 0) {
    el.innerHTML = '<p style="color:var(--text-muted);font-style:italic">ยังไม่มีข้อมูล</p>';
    return;
  }
  el.innerHTML = YOGACARA_DATA.map(buildCard).join('');
}

function updateStats() {
  const tot = YOGACARA_DATA.reduce((s, x) => s + x.chapters_count, 0);
  const done = YOGACARA_DATA.filter(x => x.status === 'completed').length;
  const wip = YOGACARA_DATA.filter(x => x.status === 'in_progress').length;
  ['total-texts', 'total-chapters', 'completed', 'in-progress'].forEach(id => {
    const el = document.getElementById(id);
    if (!el) return;
    if (id === 'total-texts') el.textContent = YOGACARA_DATA.length;
